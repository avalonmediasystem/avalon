...
$.widget( "xml.xmlEditor", {
	...
	_create: function() {
		var self = this;
		this.instanceNumber = $(".xml_editor_container").length;
		...
		if (this.options.containerElement != null) {
		    this.containerElement = this.options.containerElement['element'];
		    this.viewportFixedHeight = this.options.containerElement['fixedHeight'];
		} else {
		    this.containerElement = $(window); 
                    this.viewportFixedHeight = false;
		}
		this.viewportOffsetTop = Math.ceil(this.containerElement.offset().top);
		this.viewportOffsetLeft = Math.ceil(this.containerElement.offset().left);
		this.viewportHeight = Math.floor(this.containerElement.height());
		this.viewportWidth = Math.floor(this.containerElement.width());
	...
	constructEditor: function() {
		...
		if (this.viewportFixedHeight) {
		    ...
		    this.xmlTabContainer.height(this.viewportHeight - this.editorHeader.outerHeight());
		}
	...
	resize: function () {
		if (this.viewportFixedHeight) {
		    this.problemsPanel.css('max-height',(this.viewportHeight-this.editorHeader.outerHeight())/2+'px');
		    this.xmlTabContainer.height(this.viewportHeight - this.editorHeader.outerHeight() - this.problemsPanel.outerHeight());
		}

		this.xmlTabContainer.width(this.xmlEditorContainer.innerWidth() - this.modifyMenu.menuColumn.outerWidth());
	...
	modeChange: function(mode) {
		...
		this.editorHeader.find(".active_mode_tab").removeClass("active_mode_tab");
		...
		if (mode == 0) {
			this.activeEditor = this.guiEditor;
			this.editorHeader.find("#" + xmlMenuHeaderPrefix + this.options.xmlEditorLabel.replace(/ /g, "_")).addClass("active_mode_tab");
		} else {
			this.activeEditor = this.textEditor;
			this.editorHeader.find("#" + xmlMenuHeaderPrefix + this.options.textEditorLabel.replace(/ /g, "_")).addClass("active_mode_tab");
		}
	...
	uploadXML: function(config) {
		...
		if (this.textEditor.active) {
			try {
				...
				this.xmlState.documentChangedEvent();
				this.xmlEditorContainer.find("." + submissionStatusClass).html("Failed to submit<br/>See errors at top").css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
		...
		this.xmlEditorContainer.find("." + submissionStatusClass).html("Submitting...");
		var self = this;
		$.ajax({
			url : config.url,
			dataType: "json",
			type : "POST",
			data : {xml_content: xmlString},
			success : function(response) {
				...
				if (!outcome) {
					...
					self.xmlEditorContainer.find("." + submissionStatusClass).html("Failed to submit<br/>See errors at top").css("background-color", "#ffbbbb").animate({backgroundColor: "#ffffff"}, 1000);
					self.addProblem("XML upload failed. Errors: ", outcome);
				}
			},
	...
	addProblem: function(message, problem) {
		...
		if (problem !== undefined) {
		    if (problem.substring) this.problemsPanel.append(problem);//.replace(/</g, "&lt;").replace(/>/g, "&gt;"));
		    else this.problemsPanel.append(problem.message);//.replace(/</g, "&lt;").replace(/>/g, "&gt;"));
		}
	...
	keydownCallback: function(e) {
		if (this.guiEditor.active) {
			var focused = this.xmlTabContainer.find("input:focus, textarea:focus, select:focus");
		...
		if (e.altKey && e.shiftKey && e.keyCode == 'S'.charCodeAt(0)) {
			this.xmlEditorContainer.find("." + submitButtonClass).click();
			return false;
		}
	...
	refreshMenuUndo: function(self) {
		if (self.undoHistory.headIndex > 0) {
			self.xmlEditorContainer.find("#" + xmlMenuHeaderPrefix + "Undo").removeClass("disabled").data("menuItemData").enabled = true;
		} else {
			self.xmlEditorContainer.find("#" + xmlMenuHeaderPrefix + "Undo").addClass("disabled").data("menuItemData").enabled = false;
		}
		if (self.undoHistory.headIndex < self.undoHistory.states.length - 1) {
			self.xmlEditorContainer.find("#" + xmlMenuHeaderPrefix + "Redo").removeClass("disabled").data("menuItemData").enabled = true;
		} else {
			self.xmlEditorContainer.find("#" + xmlMenuHeaderPrefix + "Redo").addClass("disabled").data("menuItemData").enabled = false;
		}
	},
	...
	refreshMenuSelected: function(self) {
		...
		$.each(suffixes, function(){
			if (hasSelected)
				self.xmlEditorContainer.find("#" + xmlMenuHeaderPrefix + this.toString()).removeClass("disabled").data("menuItemData").enabled = true;
			else self.xmlEditorContainer.find("#" + xmlMenuHeaderPrefix + this.toString()).addClass("disabled").data("menuItemData").enabled = false;
		});
	}
});
...
DocumentState.prototype.updateStateMessage = function () {
	if (this.isChanged()) {
		this.editor.xmlEditorContainer.find("." + submissionStatusClass).html("Unsaved changes");
	} else {
		this.editor.xmlEditorContainer.find("." + submissionStatusClass).html("All changes saved");
	}
};
...
GUIEditor.prototype.initialize = function(parentContainer) {
	...
	if (this.editor.viewportFixedHeight) this.xmlContent.css('padding-right','0.5em');
	...
	if (this.editor.viewportFixedHeight) this.guiContent.css('min-height',this.editor.viewportHeight-this.editor.editorHeader.outerHeight()-20);
...
GUIEditor.prototype.clearElements = function() {
	this.editor.xmlEditorContainer.find("." + topLevelContainerClass).remove();
	return this;
};

GUIEditor.prototype.resize = function() {
        this.guiContent.width(this.editor.xmlTabContainer.width()-20);
        this.xmlContent.width(this.guiContent.innerWidth()-10);
	return this;
};
...
GUIEditor.prototype.selectElement = function(selected) {
	if (!selected || selected.length == 0) {
		...
		this.editor.xmlEditorContainer.find("." + xmlElementClass + ".selected").removeClass("selected");
		this.editor.xmlEditorContainer.find('.' + attributeContainerClass + ".selected").removeClass("selected");
...
GUIEditor.prototype.deselect = function() {
	var selectedAttributes = this.editor.xmlEditorContainer.find('.' + attributeContainerClass + ".selected");
...
GUIEditor.prototype.selectSibling = function(reverse) {
	...
	if (this.selectedElement.domNode.length > 0) {
		...
		if (!reverse)
			newSelection = this.editor.xmlEditorContainer.find("." + xmlElementClass).first();
	}
...
GUIEditor.prototype.selectNext = function(reverse) {
	...
	if (this.selectedElement == null) {
		...
		var found = false;
		var allElements = this.editor.xmlEditorContainer.find("." + xmlElementClass + ":visible", this.xmlContent);
...
GUIEditor.prototype.focusInput = function(reverse) {
	var focused = this.editor.xmlEditorContainer.find("input:focus, textarea:focus, select:focus");
...
function MenuBar(editor) {
	...
	this.headerMenuData = [ {
		...
		action : function(event) {self.activateMenu(event);}, 
		items : [ {
			label : 'Switch to '+self.editor.options.xmlEditorLabel+' View',
			enabled : true,
			binding : "alt+shift+x",
			action : function() {
			...
			label : 'Switch to '+self.editor.options.textEditorLabel+' View',
			enabled : true,
			binding : "alt+shift+t",
			action : function() {
...
ModifyMenuPanel.prototype.initialize = function (parentContainer) {
		...
		if (self.editor.submitButtonConfigs != null){
			$.each(self.editor.submitButtonConfigs, function(index, config){
				...
				if (config.id && ('createDomElement' in config) && !config.createDomElement) {
					submitButton = this.xmlEditorContainer.find("#" + config.id);
				} else {
				...
				if (!('responseHandler' in config) && config.url) {
					config.responseHandler = self.editor.options.submitResponseHandler
	...
	var statusHeight = $(this.editor.options.documentStatusPanelDomId).height();
	this.menuContainer.css({'max-height': this.editor.viewportHeight-statusHeight, 'overflow-y': 'auto'});
	return this;
};
