...
it('Verify setting Special access for an Avalon user - published item - @Ta15294e5', () => {
  ...
  .find('input#add_user_display').type(user_username).should('have.value', user_username)
...
it('Verify creating an item under a collection - @T139381a0', () => {
	...
	cy.get('div#file-upload input[type="file"][name="Filedata[]"]')
	  .selectFile(`spec/cypress/fixtures/${videoName}`, { force: true });
	cy.wait(5000);
	...
	cy.get('input#media_object_title').type(item_title).should('have.value', item_title);
	...
	cy.get('div.ramp--tabs-panel').within(() => {
	  ...
	  cy.get('div.tab-content dt').contains('Collection').next('dd').contains(collection_title);
	});
...
it('Verify that modifying the resource metadata fields are reflected properly in the preview section- @T16bc91af', () => {
  ...
  cy.get('input#media_object_creator_0').clear().type(main_contributor).should('have.value', main_contributor);
  cy.get('input#display_media_object_language_0').clear().type(language).should('have.value', language);
  cy.get('textarea#abstract_0').clear().type(summary).should('have.value', summary);
  cy.get('input[name="save_and_continue"][value="Save and continue"]').click();
  ...
  //Validate the fields
  cy.get('div.ramp--tabs-panel').within(() => {
	cy.get('div.tab-content dt').contains('Summary').next('dd')
	  .should('have.text', summary);

	// Validate the "Language" field
	cy.get('div.tab-content dt').contains('Language').next('dd')
	  .should('have.text', language);

	// Validate the "Main contributor" field
	cy.get('div.tab-content dt').contains('Main contributor').next('dd')
	  .should('have.text', main_contributor);
  });

});

it.only('Verify modifying the resource metadata of an item reflects in the index- @Tec49689f', () => {
	cy.login('administrator')
	cy.visit('/')
	// The below code is hard-coded for a media object url. This needs to be changed with a valid object URL later for each website.
	cy.visit('/media_objects/' + item_id)
  ...
  cy.get('input#media_object_genre_0').clear().type(genre).should('have.value', genre);
  cy.screenshot()
  ...
  cy.contains('a.nav-link', 'Browse').click()
	cy.get("input.global-search-input[placeholder='Search this site']").first().type(item_title).should('have.value', item_title) // Only yield inputs within form
