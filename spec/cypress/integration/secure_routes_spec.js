...
context('Secure routes', () => {
  ...
  it('.about_access_admins()', { tags: '@critical' }, () => {
  ...
  it('.about_access_regular_users()', { tags: '@critical' }, () => {
    cy.login('user');
    cy.visit('/about');
  ...
  it('.about_access_anonymous_users()', { tags: '@critical' }, () => {
    cy.visit('/about');
    cy.contains('Environment').should('not.exist');
  });

  // '/about/health' - should provide access to admins
  it('.health_access_admins()', { tags: '@critical' }, () => {
    cy.login('administrator');
  ...
  it('.health_access_regular_users()', { tags: '@critical' }, () => {
    cy.login('user');
    cy.visit('/about/health');
  ...
  it('.health_access_anonymous_users()', { tags: '@critical' }, () => {
    cy.visit('/about/health');
    cy.contains('Service Health').should('not.exist');
  });
});
