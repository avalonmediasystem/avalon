...
class IngestBatchMailer < ActionMailer::Base
  ...
  def status_email( ingest_batch_id )
    ...
    @email = @ingest_batch.email || Admin::ApplicationSetting.instance.email.notification
    mail(
      to: @email, 
  ...
  def batch_ingest_validation_error( package, base_errors )
    ...
    email = package.manifest.email || Admin::ApplicationSetting.instance.email.notification
    mail(
      to: email,
      from: Admin::ApplicationSetting.instance.email.notification,
