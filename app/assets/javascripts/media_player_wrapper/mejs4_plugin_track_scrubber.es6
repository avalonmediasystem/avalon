...
Object.assign(MediaElementPlayer.prototype, {
    ...
    trackScrubberObj: {
      ...
      initializeTrackScrubber: function (trackstart, trackend, stream_info) {
        ...
        let start_percent = Math.max(0,Math.min(100,Math.round(100*trackstart / duration)));
        let end_percent = Math.max(0,Math.min(100,Math.round(100*trackend / duration)));
        ...
        return total.bind('mousedown touchstart', (e) => {
      ...
      resizeTrackScrubber: function () {
        ...
        const timeWidth = $(scrubberEl).find('.track-mejs-currenttime-container').outerWidth(true);
        const durationWidth = $(scrubberEl).find('.track-mejs-duration-container').outerWidth(true);
        const railWidth = totalWidth - timeWidth - durationWidth - 5;
        const totalTimeWidth = railWidth - ($timeTotal.outerWidth(true) - $timeTotal.width());
      ...
      updateTrackScrubberProgressBar: function (currentTime) {
        ...
        let trackpercent = Math.min(100, Math.max(0,(100 * trackoffset / this.trackdata['trackduration'])));
