...
$(document).ready(function() {
  ...
  var import_button_html = '<div class="input-group-append"><button id="media_object_bibliographic_id_btn" type="button" name="media_object[import_bib_record]" class="btn btn-outline" value="yes" >Import</button></div>';

  $('div.import-button').append(import_button_html);

  $('#media_object_bibliographic_id').on('keyup', function() {
    ...
    if ($('input#media_object_title').val() != '' && import_val != '') {
      ...
      $(function() {
        ...
        }).popover('enable').on('click', function() {
      ...
      $(function() {
        $('#media_object_bibliographic_id_btn').popover('disable');
      });
    }
  });
});