...
$ ->
  ...
  $(document).on 'click', '.add-dynamic-field', (e) ->
    ...
    if current_input_group.find('.typeahead').length
      ...
      new_autocomplete = new_input_group.find('.typeahead')
      for_attr = new_autocomplete.attr('for')
      target = for_attr.split('-')[0].split('_')
      target.push(parseInt(target.pop()) + 1)
      target = target.join('_')
      new_autocomplete.attr('for', target + '-popup')
      ul = new_autocomplete.find(".autocomplete_popup")
      ul.attr('id', target + '-popup')
      feedback = new_autocomplete.find(".autocomplete_feedback")
