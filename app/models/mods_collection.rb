...
class ModsCollection < ActiveFedora::NokogiriDatastream    
  ...
  set_terminology do |t|
    ...
    # Generates an empty Mods Article (used when you call ModsCollection.new without passing in existing xml)
    ...
    def insert_contributor(type, opts={})
      case type.to_sym 
      when :person
        node = Hydra::ModsCollection.person_template
        nodeset = self.find_by_terms(:person)
      when :organization
        node = Hydra::ModsCollection.organization_template
        nodeset = self.find_by_terms(:organization)
      when :conference
        node = Hydra::ModsCollection.conference_template
        nodeset = self.find_by_terms(:conference)
      else
        ActiveFedora.logger.warn("#{type} is not a valid argument for Hydra::ModsCollection.insert_contributor")
        node = nil
        index = nil
      end
    ...
    def self.dc_relator_terms
      ...
      def to_solr(solr_doc=Hash.new)
        ...
        solr_doc = solr_doc.merge(object_type_hash)
        #solr_doc = solr_doc.merge(title_s_hash)
        #solr_doc = solr_doc.merge(collection_creator_hash)
        solr_doc
      end
