...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field 'read_access_group_ssim', label: 'Item access', if: Proc.new {|context, config, opts| context.current_ability.can?(:read, :administrative_facets)}, group: "workflow", query: {
      public: { label: "Public", fq: "(disable_inheritance_bsi:true AND read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC}) OR (disable_inheritance_bsi:false AND {!join from=id to=isGovernedBy_ssim}inheritable_read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC})" },
      restricted: { label: "Authenticated", fq: "(disable_inheritance_bsi:true AND read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED}) OR (disable_inheritance_bsi:false AND {!join from=id to=isGovernedBy_ssim}inheritable_read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED})" },
      private: { label: "Private", fq: "(disable_inheritance_bsi:true AND NOT read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC} AND NOT read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED}) OR (disable_inheritance_bsi:false AND NOT {!join from=id to=isGovernedBy_ssim}inheritable_read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC} AND NOT {!join from=id to=isGovernedBy_ssim}inheritable_read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED})" }
    }
