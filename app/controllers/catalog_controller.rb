...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field 'read_access_group_ssim', label: 'Item access', if: Proc.new {|context, config, opts| context.current_ability.can?(:read, :administrative_facets)}, group: "workflow", query: {
      public: { label: "Public (not inherited)", fq: "disable_inheritance_bsi:true AND read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC}" },
      restricted: { label: "Authenticated (not inherited)", fq: "disable_inheritance_bsi:true AND read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED}" },
      private: { label: "Private (not inherited)", fq: "disable_inheritance_bsi:true AND NOT read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC} AND NOT read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED}" },
      inherited: { label: "Inherited (Show inherited item access facet)", fq: "*:* AND NOT disable_inheritance_bsi:true" }
    }
    config.add_facet_field 'inheritable_read_access_group_ssim', label: 'Item access (inherited)', if: Proc.new {|context, config, opts| context.params.dig('f', 'read_access_group_ssim')&.include?('inherited') && context.current_ability.can?(:read, :administrative_facets)}, group: "workflow", query: {
      public: { label: "Public (inherited)", fq: "*:* AND NOT disable_inheritance_bsi:true AND {!graph to=id from=isGovernedBy_ssim}inheritable_read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC}" },
      restricted: { label: "Authenticated (inherited)", fq: "*:* AND NOT disable_inheritance_bsi:true AND {!graph to=id from=isGovernedBy_ssim}inheritable_read_access_group_ssim:#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED}" },
      private: { label: "Private (inherited)", fq: "*:* AND NOT disable_inheritance_bsi:true AND NOT {!graph to=id from=isGovernedBy_ssim}{!terms f=inheritable_read_access_group_ssim}#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_PUBLIC},#{Hydra::AccessControls::AccessRight::PERMISSION_TEXT_VALUE_AUTHENTICATED}" }
    }
    ...
    #config.default_solr_params[:'facet.field'] = config.facet_fields.keys # Commented out to do this in the search builder where conditionals can be evaluated
